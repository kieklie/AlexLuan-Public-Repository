...
def extract_term(text: str) -> Optional[str]:
    ...
    # 尝试提取第一个大写开头的单词（通常是英文术语；需在 lower 之前匹配）
    match = re.search(r'\b([A-Z][a-zA-Z0-9]+)\b', raw)
    if match:
        return match.group(1).lower()
...
class TerminologyAgent:
    ...
    def _filter_response(self, text: str) -> str:
        """
        过滤回复中的敏感内容，防止系统提示词泄露和思考链暴露。
        """
        if not text:
            return text

        # 1. 如果回复包含完整或部分系统提示词（非常长的AI术语专家相关文字），直接截断
        # 检测是否存在 "你是一位资深的人工智能术语专家" 或类似开头
    ...
    async def chat(self, session_id: str, user_message: str) -> dict:
        ...
        # 过滤回复中的敏感内容
        assistant_reply = self._filter_response(assistant_reply)
